...
is generated by a best-effort prebuild step. If `jupyter lite` is not already
installed, the script uses [uv](https://docs.astral.sh/uv/) to run the constrained
build requirements in an isolated, cached environment:

```bash
npm run build:jupyterlite
```

Without uv, install the requirements into your active Python environment first:
...
Both **skip gracefully** when neither `jupyter lite` nor uv is installed — a
Node-only build still succeeds and the web Notebook panel shows a "not built"
message until the site is generated. The desktop (Tauri) dev
